...
import {listUserInterfaceLeftNumByPageUsingGET} from "@/services/kaiapi-backend/userInterfaceInfoController";
import {generateOrderUsingPOST, generateTokenUsingGET} from "@/services/kaiapi-order/orderController";
import {listInterfaceInfoByPageUsingGET} from "@/services/kaiapi-backend/interfaceInfoController";
...
const Index: React.FC = () => {
  ...
  const [confirmData,setConfirmData] = useState<API.InterfaceInfo>();
  const [totalMoney,setTotalMoney] = useState<any>()
  const [inputNumberValue ,setInputNumberValue] =useState<number>(1)
  const [confirmLoading,setConfirmLoading] = useState(false)
  const columnsMap: Record<string, ProColumns<API.InterfaceInfo>[]> = {
    tab1: [
      {
        title: '请求id',
        dataIndex: 'id',
    ...
    tab2: [
      ...
      },
      {
        ...
        width: 150,
        render: (_,record) => [
          //content={'剩余可购买次数：'+record.availablePieces+'条'}
          <Popover >
  ...
  const submitOrder = async ()=>{
    ...
    try {
      const  res = await generateOrderUsingPOST({
      ...
      <ProTable<API.InterfaceInfo>
        ...
        request={async (params, sorter, filter) => {
          ...
          if (activeKey === "tab1"){
            ...
            const  res = await listUserInterfaceLeftNumByPageUsingGET({...params});
            if (res?.data) {
              setTableListDataSource(res?.data.records)
              return {
                data: res?.data?.records,
      ...
      <Modal title="确认订单" open={confirmOrderModal} onCancel={()=>{setConfirmOrderModal(false),setInputNumberValue(1)}} footer={[
        ...
        <Descriptions column={1} bordered>
          ...
          <Descriptions.Item label="购买次数" >
            <InputNumber min={1} value={inputNumberValue} max={Number(200)} defaultValue={1} onChange={ (value:any) => {setInputNumberValue(value),setTotalNumMoney(value,confirmData?.charging)}} />
          </Descriptions.Item>
