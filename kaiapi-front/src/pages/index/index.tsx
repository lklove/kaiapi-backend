...
import {
  bindPhoneUsingPOST,
  getAllUserCountUsingGET,
  getCaptchaUsingGET,
  getGithubStartUsingGET
} from "@/services/kaiapi-backend/userController";
import {getAvailableInterfaceUsingGET} from "@/services/kaiapi-backend/interfaceInfoController";
import {getOrderSuccessCountUsingGET} from "@/services/kaiapi-order/orderController";
...
const Index: React.FC = () => {
  ...
  const loadData =async()=>{
    ...
    try {
      //获取活跃用户数
      const resUser = await getAllUserCountUsingGET()
      //获取可调用接口数
      const resInterface = await getAvailableInterfaceUsingGET()
      //获取订单交易成功的数据
      const resOrderSuccess =  await getOrderSuccessCountUsingGET()
      //获取echarts中的数据
      //  const resEcharts = await getEchartsDataUsingGET()
      //获取GitHub上该项目的stars
      const resGithub = await getGithubStartUsingGET()
      ...
      <Alert type="success" message="开发不易，给孩子点个Stars吧(˚ ˃̣̣̥᷄⌓˂̣̣̥᷅ )，please~ →→→→→→→→→"  showIcon
       action={
         <Button  size="small" onClick={()=>{
           window.open("https://github.com/lklove/kaiapi-backend")
         }} type="link">
           这次一定！
         </Button>
       }
