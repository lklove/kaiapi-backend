...
import {getOrderInfoByPageUsingGET, cancelOrderSnUsingPOST} from "@/services/kaiapi-order/orderController";
...
const myOrderInfo: React.FC = () => {
  ...
  const columns: ProColumns<API.OrderDetailVo>[] = [
    ...
    },
    {
      ...
      valueEnum: {
        ...
        2: { text: '超时未支付', status: 'Error' },
        3: { text: '取消订单', status: 'Error' }
      },
    },
    {
      title: '数量',
  ...
  return (
    <PageContainer style={{position:"relative",height:'100%',width:'100%'}}>
      <ProTable<API.OrderDetailVo>
        columns={columns}
        actionRef={actionRef}
        dataSource={tableListDataSource}
        pagination={{showSizeChanger: true}}
        request={async (params) =>{
          const res = await getOrderInfoByPageUsingGET({userId:initialState?.loginUser?.id,...params})
          if (res?.data) {
